...
/**
 * 1_使用两个模板导出ppt
 * 2_删除单元格(这里还不完美解决，查看下一版本的解决办法)
 */
public class TestCreatePPT8 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileOutputStream out = new FileOutputStream("D:\\testDir\\PPT\\1_onepage\\onepage_test25.ppt");
    ...
    public static void setDataToTable(XSLFTable table) {
        ...
        for (int i = 0; i < datas.length; i++) {
            ...
            if (i == 0) {
                tableRow = table.getRows().get(0);
            } else {
                tableRow = table.addRow(); // 创建表格行
            }
            for (int j = 0; j < datas[i].length; j++) {
                XSLFTableCell tableCell = null;// 表格的单元格
                if (i == 0 && j == 0) {
                    tableCell = tableRow.getCells().get(0);
