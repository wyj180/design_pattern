 ...
 * 功能：子流程动态删除执行实例
 */
public class DeleteMultiInstanceCmd2 implements Command {
    ...
    public DeleteMultiInstanceCmd2(String taskId, boolean isNormalComplete) {
        this.taskId = taskId;
    ...
    public Object execute(CommandContext commandContext) {
        ...
        // 获取二级执行实例
        // 示例 ： 100026(根据taskId获取到的执行实例ID) --(parent)--> (100018) -> (100013(根据该执行实例可以获取正在运行的执行实例数量、总执行实例数量)) -> (100005) -> (null)
        ExecutionEntity sencondExecution = execution.getParent().getParent();

        //首先判断当前任务是否是属于多实例节点
        String processDefinitionId = sencondExecution.getProcessDefinitionId();
        BpmnModel bpmnModel = ProcessDefinitionUtil.getBpmnModel(processDefinitionId);
        Activity activityElement = (Activity) bpmnModel.getFlowElement(sencondExecution.getCurrentActivityId());
