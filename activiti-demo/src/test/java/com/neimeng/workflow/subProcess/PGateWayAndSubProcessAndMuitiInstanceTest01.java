...
/**
 * 测试通过 多流程实例子流程中使用并行网关
 */
public class PGateWayAndSubProcessAndMuitiInstanceTest01 extends WorkflowApplicationTests {
    ...
    public void deploymentProcessDefinition() {
        Deployment deployment = repositoryService
                ...
                .name("多实例子流程中使用并行网关")// 添加部署的名称
                .addClasspathResource("processes/muitiSubProcess0081.bpmn")// 从classpath的资源中加载，一次只能加载一个文件
                .deploy(); // 部署
    ...
    public void startProcessInstance() {
        // 根据流程定义的key启动流程
        String processDefinitionKey = "muitiSubProcess0081";
...
//        variables.put("userNameList", Arrays.asList("MM", "NN"));
    ...
    private void completeTask(String... taskIds) {
        for (String taskId : taskIds) {
            Map<String, Object> variables = new HashMap<>();
            variables.put("endAllSubProcess", 2);

            taskService.complete(taskId, variables);
            System.out.println("处理了任务：taskId = " + taskId);
        }
    }
    ...
    public void setVariables(){
        runtimeService.setVariable("247529", "endSubProcessLife", 1);
